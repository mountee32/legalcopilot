...
describe("Email integration accounts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("GET /integrations/email/accounts returns list shape", async () => {
    ...
    const { GET } = await import("@/app/api/integrations/email/accounts/route");
    const request = new NextRequest(
      "http://localhost/api/integrations/email/accounts?page=1&limit=20"
    );
  ...
  it("POST /integrations/email/accounts returns webhook secret", async () => {
    const { withFirmDb } = await import("@/lib/db/tenant");
    vi.mocked(withFirmDb).mockResolvedValueOnce({
      id: "a1",
      provider: "google",
    ...
    const { POST } = await import("@/app/api/integrations/email/accounts/route");
    const request = new NextRequest("http://localhost/api/integrations/email/accounts", {
      method: "POST",
      body: JSON.stringify({ provider: "google", emailAddress: "user@example.com" }),
    });
