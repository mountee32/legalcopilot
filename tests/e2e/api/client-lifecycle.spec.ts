...
test.describe("Clients API", () => {
  ...
  // For now, they test the API structure and will skip if auth fails
  ...
  test.describe("GET /api/clients", () => {
    test("returns list with pagination structure", async ({ request }) => {
      ...
      // If 401, auth not configured - skip
    ...
    test("supports status filter", async ({ request }) => {
      const response = await request.get("/api/clients?status=active");
  ...
  test.describe("POST /api/clients", () => {
    test("creates individual client", async ({ request }) => {
      ...
      // Cleanup
      if (body.id) {
        await request.delete(`/api/clients/${body.id}`);
      }
    });
...
test.describe("Client with Matter Lifecycle", () => {
  ...
  test("create client and add matter", async ({ request }) => {
    ...
    if (matterResponse.status() === 404) {
      // Matter API not implemented
      test.skip();
      return;
    }
  ...
  test.afterAll(async ({ request }) => {
    // Cleanup
    if (matterId) {
      await request.delete(`/api/matters/${matterId}`);
    }
    if (clientId) {
