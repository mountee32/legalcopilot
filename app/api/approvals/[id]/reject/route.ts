...
import { applyRejectionSideEffectsIfSupported } from "@/lib/approvals/reject";
      ...
      const updated = await withFirmDb(firmId, async (tx) => {
        ...
        if (updated?.sourceId && updated.sourceId !== user.user.id) {
          await createNotification(tx, {
            ...
            metadata: { approvalId: updated.id, status: "rejected" },
          });
        }

        if (updated) {
          await applyRejectionSideEffectsIfSupported(tx, {
