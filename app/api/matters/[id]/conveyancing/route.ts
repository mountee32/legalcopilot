/**
 * PATCH /api/matters/[id]/conveyancing
 *
 * Update conveyancing-specific data for a matter.
...
import { UpdateConveyancingDataSchema } from "@/lib/api/schemas/practice-modules";
...
export const PATCH = withErrorHandler(
  withAuth(
    withPermission("cases:write")(async (request, { params, user }) => {
      ...
      const data = UpdateConveyancingDataSchema.parse(body);

      const matter = await withFirmDb(firmId, async (tx) => {
        // First, verify the matter exists and is a conveyancing matter
        ...
        if (existing.practiceArea !== "conveyancing") {
          throw new ValidationError(
            `Matter is not a conveyancing matter (practiceArea: ${existing.practiceArea})`
          );
        }
        ...
        if (updated) {
          await createTimelineEvent(tx, {
            firmId,
            matterId: updated.id,
            type: "conveyancing_updated",
            title: "Conveyancing data updated",
            actorType: "user",
