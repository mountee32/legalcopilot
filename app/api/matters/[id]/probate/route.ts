/**
 * PATCH /api/matters/[id]/probate
 *
 * Update probate-specific data for a matter.
...
import { UpdateProbateDataSchema } from "@/lib/api/schemas/practice-modules";
...
export const PATCH = withErrorHandler(
  withAuth(
    withPermission("cases:write")(async (request, { params, user }) => {
      ...
      const data = UpdateProbateDataSchema.parse(body);

      const matter = await withFirmDb(firmId, async (tx) => {
        // First, verify the matter exists and is a probate matter
        ...
        if (existing.practiceArea !== "probate") {
          throw new ValidationError(
            `Matter is not a probate matter (practiceArea: ${existing.practiceArea})`
          );
        }
        ...
        if (updated) {
          await createTimelineEvent(tx, {
            firmId,
            matterId: updated.id,
            type: "probate_updated",
            title: "Probate data updated",
            actorType: "user",
